...
const app = express(); //通过express定义app路由
// 定义数据的读取
...
app.use('/api',apiRoutes) //通过app.use方法,路由到具体的路由
...
const devWebpackConfig = merge(baseWebpackConfig, {
  ...
  devServer: {
    ...
    //然后找到devServer,在里面添加
    before(app) {
      app.get('/api/seller', (req, res) => { // 对前台/seller的处理接口
        res.json({ //服务器返回的数据
          errno: 0, //自定义错误码
          data: seller
        })//接口返回json数据，上面配置的数据seller就赋值给data请求后调用
      }),
        app.get('/api/goods', (req, res) => { // 对前台/goods的处理
          res.json({
            errno: 0,
            data: goods
          })
        }),
        app.get('/api/ratings', (req, res) => { // 对前台/ratings的处理
          res.json({
            errno: 0,
            data: ratings
          })
